...
Subject& Subject::operator=(const Subject& sub) {
	...
	for (Observer* obs : newObservers) {
		this->_observers->push_back(obs); //shallow copy because we want to keep a reference of the observers that are attached
	}
	return *this;
}
...
void Subject::Notify(ILoggable* iLoggable) {
	...
	for (; i != _observers->end(); ++i) {
		(*i)->Update(iLoggable); //if it doesnt go here, it's because it's not attached
	}
}
